...
class TrainModuleTask(Task):
    def __init__(self, id: int, n_inputs: int, n_outputs: int, epochs: int, batch_size: int, lr: float, device: str):
        super().__init__(id)
...
if __name__ == "__main__":
    n_tasks = 10
    available_device = ["cpu"] if not torch.cuda.is_available() else [f"cuda:{i}" for i in range(torch.cuda.device_count())]
    ...
    swarm = Swarm(n_bees=3, refresh_every=5)
